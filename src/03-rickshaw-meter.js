...
export function calculateSurge(baseFare, surgeMultiplier) {
  if (typeof baseFare !== "number" || typeof surgeMultiplier !== "number" || baseFare <= 0 ||
    surgeMultiplier <= 0 ||Number.isNaN(baseFare) ||Number.isNaN(surgeMultiplier))
   {
    return 0;
  }
...
export function getDistanceDifference(from, to) {
   const fromparse=parseInt(from);
   const toparse=parseInt(to);
   if(Number.isNaN(fromparse) || Number.isNaN(toparse)) return -1;
   return Math.abs(fromparse-toparse);
}
